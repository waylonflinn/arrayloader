var loader = require('./fs-loader'),
	...
	asyncMapLimit = require('async/mapLimit');
...
function typeall(files, callback){
	...
	// use mapLimit to prevent overload on low-end servers
	asyncMapLimit(files, 3, atype, callback);
}
...
function autoloadall(){
	...
	// use mapLimit to prevent overload on low-end servers
	asyncMapLimit(tuples, 3, loadhelper, callback);
}
