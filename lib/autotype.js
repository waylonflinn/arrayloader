...
*/
exports.parse_extension = parse_extension = function(url){
	...
	if(pieces.length > 0){
		...
		if(pieces[0] === COMPRESSION_TYPE){
			compression = pieces[0];
			pieces.shift();
		}
		let ext = "." + pieces[0];
	...
	return [type, compression, dimensions];
}

exports.type = autotype = function autotype(url, callback){

	var type, compression, dimensions;

	[type, compression, dimensions] = parse_extension(url);
	if(type != null){
		//console.log(type);
		return callback(null, type, compression, dimensions);
	}
	...
	preflight(url, function(err, headers){
		...
		// TODO: implement support for compression and shape
